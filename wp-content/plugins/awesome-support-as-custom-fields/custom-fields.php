...
class WPASCF_Custom_Fields {
	...
	public function register_submenu_items() {
		add_submenu_page( 'edit.php?post_type=ticket', __( 'Custom Fields', 'wpas-cf' ), __( 'Custom Fields', 'wpas-cf' ), 'administer_awesome_support', 'custom-fields', array( $this, 'custom_fields_settings_page' ) );
	}
	...
	public function load() {
		...
		// Set the sort order of core custom fields...
		add_filter( 'ascf_pre_sort_custom_fields', array( $this, 'set_sort_order_core_fields' ), 10, 1 );
	...
	*/
	...
	public function filter_custom_fields( $fields ) {
		...
		$fields = apply_filters( 'ascf_pre_sort_custom_fields', $fields );
			...
			foreach ( $fields as $name => $field ) {				
				...
				*/
				if( isset( $field['args']['hide'] ) && "yes" === $field['args']['hide'] ){
					unset( $fields[ $name ] );
					continue ; // go to the next field since this one has been removed from the array.
				}
	...
	*/
	...
	/**
	*	
	* Remove any slashes followed by single quotes from the title field.
	*
	* This can happen if the user enters a single quote in a field title.
	* Single quotes in a field title is allowed in certain languages such 
	* as french. However, posting this back to WP causes the $_POST
	* variable to escape it by adding a "/" in front of the single quote.
	* We must therefore remove that "/".
	*
	* Note that this potentially opens up an attack venue since javascript
	* can be left in this field now. BUT you would need AS or WP admin
	* rights to get to this field anyway.
	*
	* @param $field - field to be checked
	* @since  1.1.1
 	* @return void
	*/
	private function sanitize_element( &$field, $element ) {
		if ( is_array( $field ) && array_key_exists( $element, $field ) && ! empty( $field[$element] ) ) {
			$field[$element] = $this->sanitize_option_element( $field[$element] );	
		}
	}
	
	/**
	*	
	* Remove any slashes followed by single quotes from an option/string field.	
	* See additional related information in the header of function sanitize_element
	*
	* @param $option_element - option or string to be checked
	* @since  1.1.1
 	* @return string - the stanitized option/string
	*
	*/
	private function sanitize_option_element( $option_element ) {	
			$option_element = str_replace( "\'", "'", $option_element );

			// Now we don't want to be totally stupid so lets do some basic escaping...
			$option_element = esc_textarea( $option_element ) ;
		...
		if( $this->is_options_field( $field ) && array_key_exists( 'options', $field ) && is_array( $field['options']) ) {
			foreach( $field['options'] as $option_id => $option_value ) {
				if( empty( $option_value ) ) {
					...
					$field['options'][$option_id] = $this->sanitize_option_element( $option_value ) ;
				}
			}
	...
	*/
	private function sanitize_field( &$field ){
		...
		$this->sanitize_element( $field, 'title' );
		$this->sanitize_element( $field, 'placeholder' );
		$this->sanitize_element( $field, 'default' );
		$this->sanitize_element( $field, 'desc' );
		...
		$this->sanitize_bool_field( $field['boot_strap_row_fe_start'] );
		$this->sanitize_bool_field( $field['boot_strap_row_fe_end'] );
		$this->sanitize_bool_field( $field['boot_strap_column_fe'] );
	}
    ...
    */
	...
	/**
	 * Set the sort order of core custom fields.
	 * The order is set in the TICKETS->SETTINGS->Core Custom Fields screen.
	 *
	 * Filter Hook: ascf_pre_sort_custom_fields
	 *
	 * @param array $custom_fields Registered custom fields
	 *
	 * @return array
	 */	
	public function set_sort_order_core_fields( $custom_fields ) {
		$sort_order_subject 	= wpas_get_option( 'ascf_sort_order_subject' );
		$sort_order_description	= wpas_get_option( 'ascf_sort_order_description' );
		$sort_order_product 	= wpas_get_option( 'ascf_sort_order_product' );
		$sort_order_dept 		= wpas_get_option( 'ascf_sort_order_dept' );
		$sort_order_priority 	= wpas_get_option( 'ascf_sort_order_priority' );
		$sort_order_pubflag 	= wpas_get_option( 'ascf_sort_order_pubflag' );
		$sort_order_eddorderno 	= wpas_get_option( 'ascf_sort_order_eddorderno' );
		$sort_order_eddlicense 	= wpas_get_option( 'ascf_sort_order_eddlicense' );
		
		if ($sort_order_subject) {
			if ( isset( $custom_fields['title'] ) ) {
				$custom_fields['title']['args']['order'] = $sort_order_subject;
			}
		}
		
		if ($sort_order_description) {
			if ( isset( $custom_fields['message'] ) ) {
				$custom_fields['message']['args']['order'] = $sort_order_description;
			}
		}
		
		if ($sort_order_product) {
			if ( isset( $custom_fields['product'] ) ) {
				$custom_fields['product']['args']['order'] = $sort_order_product;
			}
		}
		
		if ($sort_order_dept) {
			if ( isset( $custom_fields['department'] ) ) {
				$custom_fields['department']['args']['order'] = $sort_order_dept;
			}
		}
		
		if ($sort_order_priority) {
			if ( isset( $custom_fields['ticket_priority'] ) ) {
				$custom_fields['ticket_priority']['args']['order'] = $sort_order_priority;
			}
		}
		
		/* Public/private flag only applicable if the PUBLIC TICKETS add-on is enabled but setting it here so that everything important is in one place */
		if ($sort_order_pubflag) {
			if ( isset( $custom_fields['pbtk_flag'] ) ) {
				$custom_fields['pbtk_flag']['args']['order'] = $sort_order_pubflag;
			}
		}
		
		/* EDD add-on custom fields */
		if ($sort_order_eddorderno) {
			if ( isset( $custom_fields['edd_order_num'] ) ) {
				$custom_fields['edd_order_num']['args']['order'] = $sort_order_eddorderno;
			}
		}
		
		if ($sort_order_eddlicense) {
			if ( isset( $custom_fields['edd_product_license'] ) ) {
				$custom_fields['edd_product_license']['args']['order'] = $sort_order_eddlicense;
			}
		}				

jQuery(document).ready(function($) {
	jQuery( "#as-cf-accordion" ).accordion({
		accordion: true,
		collapsible: true,
		active: false,
	});
	...
	*/
	function getFieldsData() {
		...
		$('.wpas-cf-wrapper-object').each(function(index, element) {
			...
			fieldData.extra_wrapper_css_classes = $( element ).find('.wpas-cf-extra-wrapper-css-classes-field input').val();
			fieldData.extra_field_css_classes = $( element ).find('.wpas-cf-extra-field-css-classes-field input').val();
			fieldData.extra_wrapper_css_classes_be = $( element ).find('.wpas-cf-extra-wrapper-css-classes-be-field input').val();
			fieldData.extra_field_css_classes_be = $( element ).find('.wpas-cf-extra-field-css-classes-be-field input').val();			
			fieldData.boot_strap_row_fe_start = $( element ).find('.wpas-cf-boot-strap-row-fe-start-field select').val();
			fieldData.boot_strap_row_fe_end = $( element ).find('.wpas-cf-boot-strap-row-fe-end-field select').val();
			fieldData.boot_strap_column_fe = $( element ).find('.wpas-cf-boot-strap-column-fe-field select').val();
			fieldData.save_callback = $( element ).find('.wpas-cf-save-callback-field input').val();
			fieldData.column_callback = $( element ).find('.wpas-cf-column-callback-field input').val();
			...
			/* set field defaults */
			if( ! fieldData.column_callback ) {
				fieldData.column_callback = 'wpas_cf_value';
			}
	...
	*/
	function loadAllFields() {
		...
		.done( function( markup ) {
			$('#as-cf-accordion table,h3').remove();
			$('#as-cf-accordion').append( markup );
			jQuery( "#as-cf-accordion" ).accordion( "refresh" );
			jQuery( '#as-cf-accordion' ).accordion({
				collapsible: true,
				active: false,
			});
	...
	*/
	$("#wpas-cf-add-btn").click(function(){ 
		...
		.done( function( data ) {
			$('#as-cf-accordion').append( data );
			var latest_field = jQuery('#as-cf-accordion h3').length;
			jQuery( '#as-cf-accordion' ).accordion( "refresh" );
			if( latest_field != '' || latest_field != undefined ){
				jQuery( '#as-cf-accordion' ).accordion({
					'active' :(latest_field - 1) 
				});
			}
		});
	});
	...
	*
	*/
	$('#as-cf-accordion').on( 'change', '.wpas-cf-field-type-field select', function() {
	...
	$('#as-cf-accordion').on( 'click', '#wpas-cf-add-option', function() {
		var selector = $(this).closest('tbody');
		appentOptionMarkup( selector );
	});

	$('#as-cf-accordion').on( 'click', '#wpas-cf-remove-option', function () {
		$(this).closest('.wpas-cf-option').remove();
	});

	$('#as-cf-accordion').on( 'click', '#wpas-cf-remove', function () {
		$(this).closest('table').prev('h3').remove();
	...
	$('#as-cf-accordion').on( 'click', '#wpas-cf-add-wysiwyg-option', function() {
		var option_markup = getOptionMarkup();
